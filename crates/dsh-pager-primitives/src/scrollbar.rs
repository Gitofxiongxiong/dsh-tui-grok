...
//!
//! Derived from Grok Build's `xai-grok-pager-render/src/render/scrollbar.rs`.
//! DSH changes: replaced Grok theme lookups with caller-independent fallback
//! colors; custom styles remain available through `render_scrollbar_styled`.
//!
//! # Visual Design
//!
...
fn scrollbar_styles(is_following: bool) -> (Style, Style) {
    if is_following {
        ...
        let track_style = Style::new().bg(Color::Gray);
        let thumb_style = Style::new().fg(Color::White).bg(Color::Gray);
        (track_style, thumb_style)
    }
}
