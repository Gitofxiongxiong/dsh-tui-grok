...
//!
//! Derived from Grok Build's `xai-grok-pager-render/src/render/wrapping.rs`.
//! DSH changes: removed Grok theme/safe-buffer imports and made this module
//! standalone. Source and license details are in the crate and root NOTICE.
...
use crate::line_utils::{fit_line_to_width, push_owned_lines};
    ...
    O: Into<RtOptions<'a>>,
{
    ...
    for (idx, line) in lines.into_iter().enumerate() {
        ...
        for (l, j) in wrapped.into_iter().zip(wrapped_joiners) {
            out.push(crate::line_utils::line_to_static(&l));
            joiners.push(j);
        }
    }
...
mod tests {
    ...
    fn multi_link_paragraph_underline_boundaries() {
        // The DSH-neutral extraction does not carry Grok's safe-buffer layer.
        use ratatui::buffer::Buffer;
        use ratatui::layout::Rect;
        use ratatui::style::Modifier;
