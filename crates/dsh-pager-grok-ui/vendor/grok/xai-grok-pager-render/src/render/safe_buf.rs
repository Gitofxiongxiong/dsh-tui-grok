...
impl SafeBuf for Buffer {
    ...
    fn set_line_safe_bidi(&mut self, x: u16, y: u16, line: &Line<'_>, width: u16) {
        if y >= self.area.y && y < self.area.bottom() && x < self.area.right() {
            // A1 seam: the DSH renderer has no Grok global bidi setting.
            // Callers that require bidi own that projection; dropdown chrome
            // deliberately stays in logical order upstream as well.
            self.set_line(x, y, line, width);
        }
    }
