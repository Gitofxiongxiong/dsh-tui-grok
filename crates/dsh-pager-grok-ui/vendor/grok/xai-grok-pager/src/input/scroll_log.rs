...
//! the process temp directory; any other non-`0` value is
...
//! single file-backed diagnostic (never stderr — that is the TUI's terminal), and
//! never panic.
    ...
    fn write_line(&mut self, line: &str, flush_now: bool) {
        if let Sink::Pending(path) = &self.sink {
            match open_writer(path) {
                ...
                Err(err) => {
                    crate::diag::log("scroll-log", format!("disabled: open failed: {err}"));
        ...
        if let Err(err) = result {
            crate::diag::log("scroll-log", format!("disabled: write failed: {err}"));
            self.sink = Sink::Disabled;
        }
    }
}
...
/// A temp-directory JSONL file with the upstream timestamp convention. Also the target of the
/// `/debug log` runtime toggle ([`super::mouse::MouseScrollState`]).
pub(crate) fn default_log_path() -> PathBuf {
    let ts = chrono::Utc::now().format("%Y%m%d-%H%M%S");
    std::env::temp_dir().join(format!("dsh-scroll-log-{ts}.jsonl"))
}
