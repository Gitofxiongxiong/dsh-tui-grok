...
/// Emit a product shortcut-used event for an allowlisted binding.
///
/// Upstream talks to Grok telemetry. DSH excludes that runtime (reuse class D);
/// the allowlist in [`shortcut_used_action_label`] is kept so a later seam can
/// plug in. Call only on the commit path that handles the allowlisted id.
pub fn log_shortcut_used(key: &KeyEvent, action_id: ActionId, context: &str) {
    let _ = (key, shortcut_used_action_label(action_id), context);
}
